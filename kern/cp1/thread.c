...
/**
 * Inputs:
 *   - tid: The thread ID of the thread to join.
 * Outputs:
 *   - On success, returns the thread ID of the joined thread.
 *   - On failure, returns -1.
 * Description:
 *   This function waits for the specified thread to exit if it is a child of the
 *   calling thread. If the specified thread is not a child or does not exist, 
 *   the function returns an error.
 */

int thread_join(int tid) {
    //inputs: tid - integer thread ID
    //outputs: integer thread ID
    //Description: waits for the specified thread to exit if it is a child of the calling thread. If the specified thread is not a child or does not exist, the function returns an error -1.

    struct thread * child;

    // Check if the thread ID is valid and if the thread exists in the thread table
    if (tid < 0 || tid >= NTHR || (child = thrtab[tid]) == NULL)
        return -1;

    // Check if the child thread is a child of the current thread
    if (child->parent == CURTHR) {
        // If the child thread has not exited, wait for it to exit
        if (child->state != THREAD_EXITED) {
            condition_wait(&CURTHR->child_exit);
        }
        // Recycle the thread resources
        recycle_thread(tid);
        // Return the thread ID of the joined thread
        return tid;
    } else {
        // If the child thread is not a child of the current thread, return an error
        return -1;
    }
}
...
void condition_broadcast(struct condition * cond) {
    //inputs: cond - pointer to a condition variable
    //outputs: none
    //Description: signals all threads waiting on the condition variable to wake up and continue execution

    struct thread * thr;

    while (!tlempty(&cond->wait_list)) {
        thr = tlremove(&cond->wait_list); // remove thread from wait list
        set_thread_state(thr, THREAD_READY); // set thread state to READY
        tlinsert(&ready_list, thr); // insert thread into ready list
    }
}
...
void suspend_self(void) {
    ...
    //using _thread_swtch (in thrasm.s). Must be called with interrupts enabled. Returns when the current thread is next scheduled for execution. 
    //If the current thread is RUNNING, it is marked READY and placed on the ready-to-run list. Note that suspend_self will only return if the current thread becomes READY.

    struct thread * next_thread;
    
    // If the current thread is RUNNING, mark it as READY and place it on the ready list
    if (CURTHR->state == THREAD_RUNNING) {
        set_thread_state(CURTHR, THREAD_READY);
        tlinsert(&ready_list, CURTHR);
    }

    // Check if the ready list is empty and set the idle thread to RUNNING if it is
    if (tlempty(&ready_list)) {
        next_thread = &idle_thread;
        set_thread_state(next_thread, THREAD_RUNNING);
    } 
    else {
        // Get the next thread to run from the ready list
