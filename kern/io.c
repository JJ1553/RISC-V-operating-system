...
//define iolit function prototypes
long iolit_read(struct io_intf * io, void * buf, unsigned long len);
long iolit_write(struct io_intf * io, const void * buf, unsigned long len);
int iolit_ctl(struct io_intf* io, int cmd, void* arg);

struct io_intf * iolit_init (
    struct io_lit * lit, void * buf, size_t size)
{
    static const struct io_ops ops = { //initialize io_ops struct with read, write, and ctl functions set to iolit_read, iolit_write, and iolit_ctl
        .close = NULL,
        .read = iolit_read,
        .write = iolit_write,
        .ctl = iolit_ctl
    };
    lit->io_intf.ops = &ops; //set io_intf ops to ops
    lit->buf = buf; //set io_lit buf to buf
    lit->size = size; //set io_lit size to size
    lit->pos = 0;     //set io_lit pos to 0

    return &lit->io_intf;
}

int iolit_ctl(struct io_intf* io, int cmd, void* arg) {
    //inputs: io io_intf struct, command, argument
    //outputs: 0 on success, or negative error code
    //Description: performs control operations on the io_lit object, getting the length, position, or setting the position
...
long iolit_read(struct io_intf * io, void * buf, unsigned long len) {
    //inputs: io io_intf struct, buffer pointer, buffer length
    //outputs: number of bytes read, or negative error code
    //Description: reads len bytes from the io_lit object into the buffer

    if(io == NULL || io->ops == NULL || io->ops->read == NULL) {
        return -ENOTSUP; //if io or io->ops is null, return -ENOTSUP
    }

    struct io_lit * lit = (struct io_lit *)io;

    // Check if the read position is beyond the buffer size
...
long iolit_write(struct io_intf * io, const void * buf, unsigned long len) {
    //inputs: io io_intf struct, buffer pointer, buffer length
    //outputs: number of bytes written, or negative error code
    //Description: writes len bytes from the buffer to the io_lit object

    if(io == NULL || io->ops == NULL || io->ops->read == NULL)
        return -ENOTSUP; //if io or io->ops is null, return -ENOTSUP

    struct io_lit * lit = (struct io_lit *)io; //cast io to io_lit
    ...
    // Copy data from the buffer to the destination
    memcpy((char *)lit->buf+ lit->pos, buf, to_read);
