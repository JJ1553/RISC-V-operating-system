...
_trap_entry_from_umode:
        # input: a0 = pointer to child thread
        # output: a0 = 0
        # purpose: set up child thread to return 0 from fork
        ...
        csrrw sp, sscratch, sp          # Get the kernel stack pointer
        ld tp, 0(sp)                    # Get the thread pointer
        addi sp, sp, -34*8              # Allocate space for trap frame
        sd t6, 31*8(sp)                 # Save t6 (x31) in trap frame
        csrr t6, sscratch               # Get the user stack pointer

        sd t6, 2*8(sp)                  # Save the user stack pointer
        

        save_gprs_except_t6_and_sp      # Save all general purpose registers except sp and t6 to trap frame
        save_sstatus_and_sepc           # Save sstatus and sepc to trap frame

        # We're now in S mode, so update our trap handler address to
        # _trap_entry_from_smode.
        la t0, _trap_entry_from_smode   # Get the address of _trap_entry_from_smode
        csrw stvec, t0                  # Set the trap handler address
        ...
        call    trap_umode_cont # Call trap_umode_cont

        .global _entry_from_fork

        _entry_from_fork:
        la t0, _trap_entry_from_umode # Get the address of _trap_entry_from_umode
        csrw stvec, t0                # Set the trap handler address

        restore_sstatus_and_sepc        # Restore sstatus and sepc from trap frame
        restore_gprs_except_t6_and_sp  # Restore all general purpose registers except sp and t6 from trap frame
        
        addi    t6, sp, 34*8    # Restore original sp
        csrw    sscratch, t6    # Store the top of the kernel stack to sscratch

        ld      t6, 31*8(sp)    # Restore t6 (x31) from trap frame
        ld      sp, 2*8(sp)     # Load the user stack pointer from trap frame
        sret
...
trap_umode_cont:
        # input: a0 = pointer to child thread
        # output: none
        # purpose: set up the child thread to start executing in user mode
